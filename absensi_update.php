...
if (isset($_GET["id"])) {
    $id = (int) $_GET["id"];
    $getfile = file_get_contents('json/absen.json');
...
if (isset($_POST["id"])) {
    ...
    $post["absen_masuk"] = isset($_POST["absen_masuk"]) ? $_POST["absen_masuk"] : "";
    $post["absen_keluar"] = isset($_POST["absen_keluar"]) ? $_POST["absen_keluar"] : "";

    if ($jsonfile) {
        unset($all["records"][$id]);
        $all["records"][$id] = $post;
        $all["records"] = array_values($all["records"]);
        file_put_contents("json\absen.json", json_encode($all));
    }
                  ...
                  <nav class="sidebar-nav">
                      <ul id="sidebarnav">
                          ...
                          <?php if($akses=='1'):?>
                              ...
                              <li><a href="view_absensi.php" class="waves-effect active"><i class="fa fa-table m-r-10" aria-hidden="true"></i>Data Absensi</a></li>
                  ...
                  <div class="row page-titles">
                      <div class="col-md-6 col-8 align-self-center">
                          ...
                          <ol class="breadcrumb">
                              ...
                              <li class="breadcrumb-item"><a href="view_absensi.php">Absensi</a></li>
                          ...
                          <div class="card">
                              <div class="card-block">
                                 <?php if (isset($_GET["id"])): ?>
                                  <form method="POST" action="absensi_update.php">
                                  <div class="col-md-6">
                                   <input type="hidden" value="<?php echo $id ?>" name="id"/>
                                   <div class="form-group">
                                    <label for="inputFName">nik</label>
                                    <input type="text" class="form-control" required="required" id="inputFName" value="<?php echo $jsonfile["nik"] ?>" name="nik" placeholder="NIK" disabled="">
                                    <span class="help-block"></span>
                                   </div>
                                  
                                   
                                   <div class="form-group">
                                    <label for="inputLName">absen masuk</label>
                                    <input type="time" class="form-control" required="required" id="inputLName" value="<?php echo $jsonfile["absen_masuk"] ?>" name="absen_masuk" placeholder="Absen Masuk">
                                    <span class="help-block"></span>
                                   </div>
                                   
                                   <div class="form-group">
                                    <label for="inputLName">Absen keluar</label>
                                    <input type="time" class="form-control" required="required" id="inputLName" value="<?php echo $jsonfile["absen_keluar"] ?>" name="absen_keluar" placeholder="Absen Keluar">
                                    <span class="help-block"></span>
                                   </div>
                                   <div class="form-actions">
                                    <button type="submit" id="btnUpdate" class="btn btn-info">Update</button>
                                         <a type="button" href="view_absensi.php" class="btn btn-danger" data-dismiss="modal">Cancel</a>
                                   </div>
                                  </div>
                                  </form>
                                <?php endif; ?>        
